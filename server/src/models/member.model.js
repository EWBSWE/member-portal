...
function create(data) {
  ...
  if (Array.isArray(data)) {
    dataValid = data.map(validate).reduce((a, b) => {
      return a && b;
    }, true);
...
function update(id, data) {
  return new Promise((resolve, reject) => {
    if (
      ...
      db.one("SELECT hashed_password, salt FROM member WHERE id = $1", id)
        .then((member) => {
          if (
            !authenticate(data.password, member.hashed_password, member.salt)
          ) {
...
function extendMembership(member, product) {
  ...
  if (member.expiration_date !== null) {
    expirationDate = moment(member.expiration_date)
      .add(product.attribute.days, "days")
      .toDate();
  }
